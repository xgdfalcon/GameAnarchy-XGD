...
namespace GameAnarchy {
    internal static class GALocale {
        private const ulong m_betaModID = 2781804786uL;
        private const ulong m_thisModID = 2785207458uL;
        private const string m_fileNameTemplate = @"GameAnarchy.";
        private const string m_defaultFile = @"GameAnarchy.en.locale";
        ...
        private static void LoadLocale(string culture) {
            ...
            try {
                ...
                using (MemoryStream ms = new MemoryStream(DefaultLocale.GameAnarchy_en)) {
                    locale.Load(ms);
                }
            } finally {
                m_xmlLocale = locale;
            ...
            if (!isInitialized) {
                try {
                    ...
                    if (m_directory is null) {
                        string dir = DataLocation.modsPath + Path.DirectorySeparatorChar + @"GameAnarchy" + Path.DirectorySeparatorChar + @"Locale" + Path.DirectorySeparatorChar;
                        if (Directory.Exists(dir) && File.Exists(dir + m_defaultFile)) {
                            m_directory = dir;
                        }
                    }
